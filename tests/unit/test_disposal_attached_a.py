...
def test_export_attached_a_builds_afir_code_from_user_and_disposal_ids(monkeypatch):
  ...
  assert rendered['codice_afir'] == '10-AFIR-7'
  assert 'codice_afir' not in rendered['rows'][0]
...
def test_afir_column_is_rendered_only_in_attached_a():
  ...
  with app.app_context():
    attached_a = render_template(
      'disposal_attached_a.html', disposal=disposal, rows=[row], codice_afir='10-AFIR-7', total=2
    )
