...
def test_reschedule_to_transport_uses_order_schedule_transport_in_session(seeded_db):
  # Il delivery ha un borderò storico con un altro mezzo: il rilascio deve
  # comunque puntare al mezzo del borderò a cui è agganciato l'ordine.
  ...
  with Session() as session:
    ...
    reschedule_products(order, {product_name: {'release_collection_point_id': 0}}, session)
    product_id = product.id
    transport_id = current_transport.id
    session.commit()
...
def test_reschedule_without_order_schedule_leaves_product_unchanged(seeded_db):
