...
from ..database.schema import DeliveryGroup, User
...
def assign_delivery_group_user(user: User):
  delivery_user: User = get_by_id(User, request.json['user_id'])
...
def get_delivery_groups(user: User):
  delivery_groups = []
...
def format_query_result(tupla: tuple[DeliveryGroup, User], list: list[dict], role: UserRole) -> list[dict]:
  for element in list:
...
def query_delivery_groups() -> list[tuple[DeliveryGroup, User]]:
  with Session() as session:
    return session.query(DeliveryGroup, User).outerjoin(User, DeliveryGroup.id == User.delivery_group_id).all()
