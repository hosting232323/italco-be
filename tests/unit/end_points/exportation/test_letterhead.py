...
from src.end_points.exportation.utils import get_company_logo

from tests.unit.factories import create_company


TEMPLATE = '{% from "components/company.html" import company_header %}{{ company_header(company, company_logo) }}'
...
def test_letterhead_shows_identity_vat_and_tax_code_on_one_line(app, db):
  ...
  html = _render(app, company=company, company_logo='data:image/png;base64,AAAA')
  ...
  assert 'Powered by' in html and 'Ares Logistics' in html
...
def test_letterhead_falls_back_to_name_and_hides_empty_fields(app, db):
  ...
  html = _render(app, company=company, company_logo=None)
...
def test_letterhead_without_company_renders_nothing(app, db):
  assert _render(app, company=None, company_logo=None).strip() == ''
