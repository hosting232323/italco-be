...
def get_user_by_email(email: str) -> User | None:
  # Lookup di autenticazione, e unico punto in cui l'email viene risolta:
  ...
  with scope(company_id=None), Session() as session:
    return session.query(User).options(joinedload(User.company)).filter(User.email == email).first()
