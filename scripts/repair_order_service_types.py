  ...
  with args.output.open('x', encoding='utf-8') as output:
    ...
    try:
      with Session(engine, expire_on_commit=False) as session:
        if session.execute(text('SELECT version_num FROM alembic_version')).scalar_one() != '056':
          raise ValueError('Eseguire con schema alla revisione 056; la bonifica non applica migrazioni.')
        session.rollback()
        if args.apply:
