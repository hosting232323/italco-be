...
Un'attività può smaltire RAEE da più sedi, ciascuna con la propria iscrizione
all'Albo Gestori Ambientali e il proprio luogo di raggruppamento: i due campi
che oggi vivono su company (rae_registration/rae_grouping_place, uno solo per
attività) diventano righe di una nuova tabella figlia, rae_disposal_place.

Backfill in tre passi, come già fatto per company_id in 050:
1. una riga per ogni company con modulo RAEE acceso (rae = true), coi valori
   legali che aveva (anche se mai compilati: restano nullable, com'erano su
   company). Il filtro è sul flag e non sui due campi perché una company può
   avere disposal senza aver mai valorizzato rae_registration/rae_grouping_place;
...
3. NOT NULL sulla colonna, poi le due colonne vengono tolte da company.
...
def upgrade() -> None:
  op.create_table(
    ...
    sa.Column('rae_registration', sa.String(), nullable=True),
  ...
  bind.execute(
    sa.text("""
      INSERT INTO rae_disposal_place (name, rae_registration, rae_grouping_place, company_id, created_at, updated_at)
      SELECT :name, rae_registration, rae_grouping_place, id, now(), now()
      FROM company
      WHERE rae = true
    """),
  ...
  op.drop_column('company', 'rae_registration')
  op.drop_column('company', 'rae_grouping_place')


def downgrade() -> None:
  op.add_column('company', sa.Column('rae_grouping_place', sa.String(), nullable=True))
  op.add_column('company', sa.Column('rae_registration', sa.String(), nullable=True))

  bind = op.get_bind()
  bind.execute(
    sa.text("""
      UPDATE company c SET rae_registration = p.rae_registration, rae_grouping_place = p.rae_grouping_place
      FROM rae_disposal_place p
