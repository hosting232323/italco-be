...
def assign_customer_group_user(_):
...
def get_customer_groups(_):
  customer_groups = []
  for tupla in query_customer_groups():
    customer_groups = format_query_result(tupla, customer_groups)
  return {'status': 'ok', 'customer_groups': customer_groups}


def format_query_result(tupla: tuple[CustomerGroup, User], list: list[dict]) -> list[dict]:
  for element in list:
    if element['id'] == tupla[0].id:
      if tupla[1]:
        element['users'].append(tupla[1].format_user())
      return list

  output = tupla[0].to_dict()
  output['users'] = []
  if tupla[1]:
    output['users'].append(tupla[1].format_user())
  list.append(output)
  return list
