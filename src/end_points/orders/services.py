...
def create_order_service_user(order: Order, products: dict, user_id: int):
  service_users = query_service_users(
      list(
          set(service['id'] for services in products.values()
              for service in services)), user_id, order.type)
...
def update_order_service_user(order: Order, products: dict, user_id: int):
  ...
  for product in products.keys():
    if len([
        order_service_user for order_service_user in order_service_users
        if order_service_user.product == product
    ]) > 0:
      continue
